...
if __name__ == '__main__':
    ...
    test_data = MyDataSet(csv_file='../data/testDataSet.csv', train=False)
    ...
    model = our_model.our_CNN().to(device)
    ...
    for i in range(epoch):  # 一共进行epoch轮训练，包含训练和测试两个小步骤
        ...
        # 每训完一个epoch就存一次
        torch.save(model, f"../models-vgg/model_epoch{i}.pth")
        print("model saved")
    ...
    with torch.no_grad():
        for x, _ in test_dataloader:
            x = x.to(device)
            predict_y = model(x).item()
            print(round(predict_y, 0), file=fp)
    fp.close()
