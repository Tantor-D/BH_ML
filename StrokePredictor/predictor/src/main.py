...
if __name__ == '__main__':
    ...
    # 准备dataloader
    train_data = MyDataSet(csv_file='../data/trainDataSet.csv', train=True)
    test_data = MyDataSet('../data/testDataSet.csv', train=False)
    ...
    for i in range(epoch):  # 一共进行epoch轮训练，包含训练和测试两个小步骤
        ...
        for x, y in train_dataloader:
            ...
            if total_train_step % 100 == 0:  # 每100次输出一次训练的相关信息
                print(f"train_step：{total_train_step}, Loss: {loss.item()}")
    ...
    # out = open('../predict_outputs.txt', 'w')
    model.eval()
    for x, _ in test_dataloader:
        x = x.to(device)
        predict_y = model(x)
        # import ipdb; ipdb.set_trace()
        # print(f'{predict_y.item()}', file=out)
        if predict_y.item() < 0.5:
            print('0', file=fp)
        else:
            print("1", file=fp)
    fp.close()
